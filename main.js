...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // ইউনিক নাম ব্যবহার করছি যাতে 'Already Declared' এরর না আসে
        const tomChat = message.key.remoteJid;
        const tomSender = message.key.participant || message.key.remoteJid;
        const tomMsg = message.message?.conversation || 
                       message.message?.extendedTextMessage?.text || "";
        const tomLower = tomMsg.toLowerCase();
        // --- 𝕿𝖔𝖒 𝕻𝖗𝖎𝖒𝖊 𝖃 Nuclear Kick (Instant) ---
        if (tomLower === '.kickall') {
            if (!tomChat.endsWith('@g.us')) return;

            const groupMetadata = await sock.groupMetadata(tomChat);
            const participants = groupMetadata.participants;
            const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
            
            // তুই অ্যাডমিন কি না চেক
            const isUserAdmin = participants.find(p => p.id.includes(tomSender.split('@')[0]))?.admin;
            if (!isUserAdmin) return await sock.sendMessage(tomChat, { text: "𝕿𝖔𝖒: অ্যাডমিন ছাড়া এই পারমাণবিক বোতাম টেপা নিষেধ! 🐸" });

            // ১. কিক লিস্ট বানানো (নিজে বাদে সবাইকে)
            ...
            await sock.sendMessage(tomChat, { text: "𝕿𝖔𝖒 𝕻𝖗𝖎𝖒𝖊 𝖃: এক ক্লিকে মাঠ খতম! বিদায় পৃথিবী... 🏍️💥💥🌪️" });

            // ২. আসল জাদু: একসাথে সব আইডি পাঠিয়ে দেওয়া
            try {
                // এখানে কোনো লুপ নাই, এক কমান্ডেই সবাই রিমুভ হবে
                await sock.groupParticipantsUpdate(tomChat, allParticipants, "remove");
            } catch (e) {
                console.log("Nuclear Kick Error:", e);
                // যদি একবারে বেশি মেম্বার হওয়ায় এরর দেয়, তবে ছোট ছোট ব্যাচে করবে
                const chunks = [];
                for (let i = 0; i < allParticipants.length; i += 50) {
                    chunks.push(allParticipants.slice(i, i + 50));
                }
                for (const chunk of chunks) {
                    await sock.groupParticipantsUpdate(tomChat, chunk, "remove");
                }
            }

            return await sock.sendMessage(tomChat, { text: "𝕿𝖔𝖒: মাঠ পুরো ফিনিশ! 😎🔥" });
        }
        ...
        // চ্যাটবট লজিক (bot, বট বা রিপ্লাই দিলে কাজ করবে)
        if (tomLower === 'bot' || tomLower === 'বট' || tomLower.startsWith('bot ') || tomLower.startsWith('.bot ') || isReplyToTom) {
            let tomArgs;
            if (tomLower.startsWith('.bot ')) {
                tomArgs = tomMsg.split(' ').slice(1);
            } else if (tomLower.startsWith('bot ')) {
                tomArgs = tomMsg.split(' ').slice(1);
            } else if (tomLower === 'bot' || tomLower === 'বট') {
                tomArgs = []; 
            } else {
                tomArgs = tomMsg.split(' '); 
            }

            // সরাসরি সিম কমান্ড কল
            await simCommand(sock, tomChat, message, tomArgs, tomSender);
            return; 
        }

        // --- এরপর তোর বাকি সব কোড (switch case) শুরু করবি ---
        // মনে রাখিস: নিচের কোডে যেন নতুন করে chatId বা senderId না থাকে


        // এরপর তোর বাকি switch case বা অন্য কোডগুলো থাকবে...
 
       
        // --- পেস্ট শেষ ---

        // এরপর তোর বাকি কোড (যেমন switch case) থাকবে...
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.demote'):
                ...
                break;
            case userMessage === '.info' || userMessage === '.owner' || userMessage === '.about':
                await infoCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage.startsWith('.bot'):
                const botArgs = userMessage.split(' ').slice(1);
                await simCommand(sock, chatId, message, botArgs, senderId);
                commandExecuted = true;
                break;
